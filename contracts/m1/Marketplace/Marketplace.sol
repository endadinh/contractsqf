...
interface IFactory { 
    ...
    function getLockedItem(uint tokenId) view external returns (StructLib.ingameItems memory);
    function setOwnerItemInfo(address from, address newOwner, uint tokenId) external;
...
contract NFTMarket is ReentrancyGuard,MarketplaceStorage, Ownable {
  ...
  mapping(uint256 => MarketItem) private idToMarketItem;
    ...
    IERC721 public mainNFTs;
      ...
      constructor(address _mainToken, address _mainNFT) payable {
...
//   }
    ...
    function sellItemNeee(
        ...
        StructLib.ingameItems memory _item = factory.getLockedItem(tokenId);
        return _item;
    }
  ...
  function createMarketSale(
    ...
    StructLib.ingameItems memory item = factory.getLockedItem(tokenId);
   ...
   function delistItem(uint256 itemId) public {
    ...
    function _safeTransferToMarket(uint256 tokenId) private {
            factory.setOwnerItemInfo(payable(address(msg.sender)),address(this), tokenId);
    ...
    function _safeBuyItem(uint256 itemId,uint256 amount, uint256 tokenId ) private { 
        ...
        factory.setOwnerItemInfo(address(this), address(msg.sender), tokenId);
