...
contract FIFAStaking {
    ...
    bool public packageStakeSet;
    uint256 public pakageStakePrice;
     ...
     * @dev Throws if timestamp already set.
     */
    modifier packageStakeNotSet() {
        require(packageStakeSet == false, "The time stamp has already been set.");
        _;
    }
     ...
     * @dev Throws if timestamp not set.
     */
    modifier packageStakeIsSet() {
        require(packageStakeSet == true, "Please set the time stamp first, then try again.");
        _;
    }
    ...
    function setPakageStake(uint256 _pricePakageStake) public onlyOwner packageStakeNotSet  {
        packageStakeSet = true;
        pakageStakePrice = _pricePakageStake * 10 ** 18;
    }
    ...
    function stakeTokens(IERC20 token) public timestampIsSet packageStakeIsSet noReentrant {
        ...
        require(pakageStakePrice <= token.balanceOf(msg.sender), "Not enough STATE tokens in your wallet !");

        uint256 ticketId = _ticketIds.current();
        Staker storage staker = stakers[msg.sender];
        ticketStack storage ticket = tickets[ticketId];
        token.safeTransferFrom(msg.sender, address(this), pakageStakePrice);
        isStaking[msg.sender] = true; 
        balances[msg.sender] = balances[msg.sender].add(pakageStakePrice);

         /// update ticket info

        ticket.staker = address(msg.sender);
        ticket.ticketPeriod = block.timestamp.add(timePeriod);
        ticket.amount = pakageStakePrice;
        ...
        emit tokensStaked(msg.sender, pakageStakePrice);
    }
    ...
    function unstakeTokens(IERC20 token,uint8 ticketId) public timestampIsSet packageStakeIsSet noReentrant {
        ...
        require(isStaking[msg.sender] = true, "You are not staking");
            ...
            require(ticket.staker == address(msg.sender), "You are not owner of this ticket ");
            require(ticket.ticketPeriod <= block.timestamp , "Tokens are only available after correct time period has elapsed");
            require(ticket.isActive == false,"You claimed this ticked");
            balances[msg.sender] = balances[msg.sender].sub(ticket.amount);
            token.safeTransfer(msg.sender, ticket.amount);
     ...
     function useTicket(uint8 ticketId) public  noReentrant {
            Staker storage staker = stakers[msg.sender];
            ticketStack storage ticket = tickets[ticketId];

            require(ticket.staker == address(msg.sender), "You are not owner of this ticket ");
            require(ticket.ticketPeriod <= block.timestamp , "Tokens are only available after correct time period has elapsed");
            require(ticket.isActive == true, "This ticket is not valid ");
            require(ticket.isUsed == false, "This ticket has already been used !");
