...
contract OBAToken is ERC20("MOBATECH", "OBA", 81*10**18*10**18, 81*10**6*10**18) {
    ...
    mapping(uint8 => uint256) public packagePrice;
    mapping(address => uint256) public activeTime;
    mapping(uint8 => uint8) public rateMonthly;
    mapping(uint8 => uint8) public rateReward;
    ...
    function adminSettingPrice(uint256[7] memory price) public onlyOwner { 
        packagePrice[0] = price[0] * 10 ** 18;
        packagePrice[1] = price[1] * 10 ** 18;
        packagePrice[2] = price[2] * 10 ** 18;
        packagePrice[3] = price[3] * 10 ** 18;
        packagePrice[4] = price[4] * 10 ** 18;
        packagePrice[5] = price[5] * 10 ** 18;
        packagePrice[6] = price[6] * 10 ** 18;
    }

    function adminSettingRateMonthly(uint8[7] memory rate) public onlyOwner { 
        rateMonthly[0] = rate[0];
        rateMonthly[1] = rate[1];
        rateMonthly[2] = rate[2];
        rateMonthly[3] = rate[3];
        rateMonthly[4] = rate[4];
        rateMonthly[5] = rate[5];
        rateMonthly[6] = rate[6];
    }

    function adminSettingRateReward(uint8[7] memory rate) public onlyOwner { 
        rateReward[0] = rate[0];
        rateReward[1] = rate[1];
        rateReward[2] = rate[2];
        rateReward[3] = rate[3];
    ...
    function availableBalance(address sender) public view returns(uint256) { 
        // uint256 buyedMonth = ( block.timestamp - activeTime[sender] ) / 60 / 60 / 24 / 30;
        uint256 buyedMonth = checkMinute();
        uint256 rateUnlock = buyedMonth * rateMonthly[activePackage[sender]];
        uint256 unlockedToken;
        if(rateUnlock >= 100) { 
            unlockedToken = tokenLocked[sender];
        }
        else { 
            unlockedToken = ( tokenLocked[sender] *  rateUnlock) / 100 ;
        }
        uint256 availBalance = ( _balances[msg.sender] - tokenLocked[sender] ) + unlockedToken ;
        return availBalance;
    }

    function checkSecond() public view returns(uint256) {
        uint256 buyedTime = ( block.timestamp - activeTime[msg.sender] );
        return buyedTime;
    }

    function checkMinute() public view returns(uint256) {
        uint256 buyedTime = ( block.timestamp - activeTime[msg.sender] ) / 60;
        return buyedTime;
    }

    function checkHour() public view returns(uint256) {
        uint256 buyedTime = ( block.timestamp - activeTime[msg.sender] ) / 60 / 60;
        return buyedTime;
    }

    function checkDay() public view returns(uint256) {
        uint256 buyedTime = ( block.timestamp - activeTime[msg.sender] ) / 60 / 60 / 24;
        return buyedTime;
    }

    function checkMonth() public view returns(uint256) {
        uint256 buyedTime = ( block.timestamp - activeTime[msg.sender] ) / 60 / 60 / 24 / 30;
        return buyedTime;
    }
	...
	function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if(isActive[sender] == true) { 
            uint256 senderBalance = availableBalance(sender);
    ...
    function mintPackage(address _to, uint256 _amount,uint8 package) public onlyMinter { 
        require(totalSupply().add(_amount) <= maxSupply(), "ERC20Capped: max supply exceeded");
        tokenLocked[_to] = _amount;
        activePackage[_to] = package;
        isActive[_to] = true;
        activeTime[_to] = block.timestamp;
		_mint(_to, _amount);
    }
...
contract Token_Factory is Ownable {
    using SafeMath for uint256;
	using Address for address;

	IERC20 public _BUSD;
    OBAToken public _mainToken;
    uint256 _totalSupply = 81*10**6*10**18;
	uint256 public _maxTokenSale;
	uint256 public _saledToken;
	bool public _saleStatus;
	uint256 public _tokenPrice;
	uint256 public _countBuyer;
	uint256 public _countFunder;

    string[7] public _package;

    address public _receiveBUSDAddress;


    mapping(address => uint256) public buyedPackage;
	mapping(address => uint256) public claimedPercent;

    
    constructor(address _token) public {
		_BUSD = IERC20(address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56));
        _receiveBUSDAddress = address(msg.sender);
		_mainToken = OBAToken(_token) ;
		_maxTokenSale = 81*10**6*10**18;
		_saledToken = 0;
		_saleStatus = true;
		_tokenPrice = 8 * 10 ** 16;
	}
    ...
    function setReceivePrivateAddress(address to) public onlyOwner { 
        _receiveBUSDAddress = to;
    }


	function buyPackage( uint8 package) public { 
        require(_mainToken.isActive(address(msg.sender)) == false, "Actived Package Address !");
		// _BUSD.transferFrom(msg.sender, _receiveBUSDAddress,_mainToken.packagePrice(package) );
        uint256 tokenReward = (_mainToken.packagePrice(package).mul(_mainToken.rateReward(package)).div(100) * 10 ** 18).div(_tokenPrice) ;
		uint256 totalToken = ( _mainToken.packagePrice(package) * 10 ** 18  / _tokenPrice ); 
		_mainToken.mintPackage(address(msg.sender), ( totalToken + tokenReward ), package);  
		emit BuyPackage(address(msg.sender), package);
	}
    ...
    event BuyPackage(address indexed user, uint8 package);
